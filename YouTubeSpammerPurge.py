...
def print_prepared_comments(comments, j, logMode):
    ...
    results = youtube.comments().list(
        part="snippet",
        id=comments,
        # The API request can take an entire comma separated list of comment IDs (in "id" field) to return info about
        maxResults=100,  # 100 is the max per page, but multiple pages will be scanned
    ...
    # Index when going through comments
    for i, item in enumerate(results["items"]):
        ...
        if logMode == True:
            ...
            logFile.write(
                "     > Direct Link: " + "https://www.youtube.com/watch?v=" + videoID + "&lc=" + comments[i] + "\n\n")
...
def check_deleted_comments(commentsDictionary):
    ...
    for key, value in commentsDictionary.items():
        ...
        if results["items"]:  # Check if the items result is empty
            print("Possible Issue Deleting Comment: " + str(
                key) + " |  Check Here: " + "https://www.youtube.com/watch?v=" + str(value) + "&lc=" + str(key))
            i += 1
...
def print_count_stats(final):
    if final == True:
        print("Top Level Comments Scanned: " + str(scannedCommentsCount) + " | Replies Scanned: " + str(
            scannedRepliesCount) + " | Spam Found So Far: " + str(len(spamCommentsID)) + "\n")
    else:
        print("Top Level Comments Scanned: " + str(scannedCommentsCount) + " | Replies Scanned: " + str(
            scannedRepliesCount) + " | Spam Found So Far: " + str(len(spamCommentsID)), end="\r")
...
if __name__ == "__main__":
    ...
    if spammer_channel_id == userChannelID and mode == "2":
        ...
        print(
            "If you are testing and want to scan and/or delete your own comments, enter 'Y' to continue, otherwise enter 'N' to exit.")
        confirmation = confirm_continue("Continue?")
    ...
    try:
        ...
        if deletionEnabled == "HalfTrue":  # Check if deletion functionality is elegible to be enabled
            ...
            input(
                "\nDeletion functionality disabled for this mode because you scanned your own channel. Press Enter to "
                "exit...")
            exit()
        else:
            input(
                "\nFAILSAFE: For an unknown reason, the deletion functionality was not enabled. Cannot delete "
                "comments. Press Enter to exit...")
            exit()

        # Only proceed if deletion functionality is enabled, and user has confirmed deletion
        if confirmDelete == "YES" and deletionEnabled == "True":
            print("\n")
